...
impl EncodedSwapValue {
    fn new(four_byte_value: U256, mem_offset: U256, byte_shift: U256) -> Self {
        Self { four_byte_value, mem_offset, byte_shift }
    }
...
impl SandwichLogicV2 {
    pub fn new(weth_address: Address) -> Self {
        ...
        SandwichLogicV2 { jump_labels, weth_address }
    }
    ...
    pub fn create_payload_weth_is_input(
        ...
        let encoded_swap_value =
            encode_four_bytes(amount_out, true, self.weth_address < other_token);

        let swap_type = self._find_swap_type(true, other_token);

        let (payload, _) = encode_packed(&[
            PackedToken::NumberWithShift(swap_type, TakeLastXBytes(8)),
            PackedToken::Address(pair.address),
            PackedToken::NumberWithShift(encoded_swap_value.mem_offset, TakeLastXBytes(8)),
            PackedToken::NumberWithShift(encoded_swap_value.four_byte_value, TakeLastXBytes(32)),
        ]);

        let encoded_call_value = amount_in / *WETH_ENCODE_DIVISOR;
    ...
    pub fn create_payload_weth_is_output(
        ...
        let encoded_swap_value =
            encode_four_bytes(amount_in, false, self.weth_address < other_token);
        ...
        let (payload, _) = encode_packed(&[
            PackedToken::NumberWithShift(swap_type, TakeLastXBytes(8)),
            PackedToken::Address(pair.address),
            PackedToken::Address(other_token),
            PackedToken::NumberWithShift(encoded_swap_value.mem_offset, TakeLastXBytes(8)),
            PackedToken::NumberWithShift(encoded_swap_value.four_byte_value, TakeLastXBytes(32)),
        ]);

        let encoded_call_value = amount_out / (*WETH_ENCODE_DIVISOR);

        (payload, encoded_call_value)
    }

    fn _find_swap_type(&self, is_weth_input: bool, other_token_addr: Address) -> U256 {
        let weth_addr = self.weth_address;
...
pub fn encode_intermediary_with_dust(
    ...
    let mut backrun_in =
        encode_four_bytes(amount_in, is_weth_input, weth_address < intermediary_address);
...
pub fn decode_intermediary(
    ...
    let encoded = encode_four_bytes(amount_in, is_weth_input, weth_address < intermediary_address);
    encoded.decode()
}
