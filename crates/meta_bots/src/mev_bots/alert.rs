...
pub async fn alert_bundle<'a>(
    ...
    let response_status =
        if is_bundle_included { String::from("🟩") } else { String::from("🟥") };
    ...
    tokio::spawn(async move {
        let res = client.post(LARK_WEBHOOK.clone()).json(&bundle_notif).send().await;
        match res {
            Ok(_) => {}
            Err(err) => {
                error!("Could not send alert to discord, err: {}", err);
...
pub async fn alert_poison(malicious_token: Address, malicious_opcodes: Vec<OpCode>) {
    ...
    tokio::spawn(async move {
        let res = client.post(LARK_WEBHOOK.clone()).json(&map).send().await;
