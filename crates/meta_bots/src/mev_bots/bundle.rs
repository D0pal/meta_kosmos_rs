...
impl BundleSender {
    ...
    pub async fn new() -> Self {
        Self { pending_sandwiches: HashMap::new() }
    }
        ...
        for (target_pool, recipes) in self.pending_sandwiches.iter() {
            ...
            let target_pool = target_pool.clone();
            {
                ...
                tokio::spawn(async move {
                    ...
                    let initial_db =
                        to_cache_db(&combined_state_diffs, fork_block, &client).await.unwrap();
                    let mut fork_factory =
                        ForkFactory::new_sandbox_factory(client.clone(), initial_db, fork_block);

                    // create raw ingredients
                    let meats = recipes.iter().flat_map(|recipe| recipe.meats.clone()).collect();

                    let raw_ingredients = match RawIngredients::new(
                        &target_pool,
                        meats,
                        sandwich_state.weth_address,
                        combined_state_diffs,
                        sandwich_state.sandwidth_contract_address,
                        sandwich_state.searcher_address,
                    )
                    .await
                    {
                        Ok(recipe) => recipe,
                    ...
                    let mut optimal_sandwich = match create_optimal_sandwich(
                        &raw_ingredients,
                        weth_balance,
                        &next_block,
                        &mut fork_factory,
...
pub async fn send_bundle(
    ...
    let raw_signed_backrun_tx =
        sign_eip1559(back_slice_request, &sandwich_maker.searcher_wallet).await?;
    ...
    for relay in relay::get_all_relay_endpoints(client.clone()).await {
        let sandwich_state = sandwich_state.clone();
        let sandwich_maker = sandwich_maker.clone();
        ...
        tokio::spawn(async move {
            ...
            let is_bundle_included = match pending_bundle.await {
                ...
                Err(e) => {
                    error!("{:?} Bundle rejected due to error : {:?}", recipe.print_meats(), e);
                    false
                }
            };
            ...
            if relay.relay_name == "flashbots" {
                ...
                match (is_bundle_included, recipe.has_dust) {
                    (true, false) => {
                        let other_token = if recipe.target_pool.token_0 != weth_address {
                        ...
                        *nonce += U256::from(2);
                        info!("Adding new dust: {:?}", other_token);
                    }
                    (true, _) => {
                        // update weth balance
                        sandwich_state.update_weth_balance(profit).await;
