...
pub async fn create_ws() -> Arc<Provider<Ws>> {
    let ws = Ws::connect(
        "wss://methodical-blissful-mountain.quiknode.pro/4df963ef1e5c95b428222987008370d3ee3c3f27/",
    )
    .await
    .unwrap();
...
pub async fn create_v2_pool(pool_address: Address, client: &Arc<Provider<Ws>>) -> Pool {
    let pool_contract = UniswapV2Pair::new(pool_address, client.clone());
    ...
    Pool::new(
        ...
        token_1,
        U256::from(3000),
        meta_common::enums::PoolVariant::UniswapV2,
    )
}
...
pub fn mutate_weth_balance(
    ...
    let val = u256_to_h256_be(U256::from("100000000000000000000"));
    // Give our acc a fuckton of WETH
    state.account(weth_address).store(key, val);
    state.account(address_to_mutate).balance(mutate_amount);
}

pub async fn get_weth_balance_at_block(weth_address: Address, owner: Address, block: u64) -> U256 {
    let (fork, _instance) = create_fork_ws(block).await;
    let weth_contract = ERC20::new(weth_address, fork);
