...
pub async fn get_from_txs(
    ...
    let req = meats.iter().map(|tx| (tx, vec![TraceType::StateDiff])).collect();
...
pub fn extract_pools(
    ...
    let weth_state_diff = &state_diffs.get(&weth_address)?.storage;
...
pub async fn to_cache_db(
    ...
    for (address, acc_diff) in state.iter() {
        ...
        let future = async move {
            let nonce_ret = nonce_provider.get_transaction_count(addy, block_num).await;
            let nonce = match nonce_ret {
                Ok(nonce) => nonce,
                Err(err) => panic!("error in get tx count for address {:?}, {:?}", addy, err),
            };

            let balance =
                balance_provider.get_balance(addy, block_num).await.expect("error in get balance");
