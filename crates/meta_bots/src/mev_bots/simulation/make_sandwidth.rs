...
use crate::{
    forked_db::{ForkDB, ForkFactory},
    mev_bots::{
        braindance,
        helpers::{
            attach_braindance_module, convert_access_list, get_amount_out_evm, get_balance_of_evm,
            setup_block_state,
        },
...
use meta_dex::{
    oracle::BlockInfo,
    sandwidth::{
        v2::{decode_intermediary, encode_intermediary_with_dust, encode_weth},
        SandwichMaker,
    },
};
...
use super::{
    inspectors::{
        ...
        is_sando_safu::{IsSandoSafu, SalmonellaInspectoooor},
    },
    SimulationError,
};
...
async fn juiced_quadratic_search(
    ...
    #[cfg(test)]
    {
        ...
        crate::mev_bots::helpers::inject_sando(
            network,
            &mut fork_factory,
            upper_bound,
            sandwidth_contract_address,
    ...
    loop {
        ...
        let revenues =
            revenues.into_iter().map(|r| r.unwrap().unwrap_or_default()).collect::<Vec<_>>();
        ...
        lower_bound = left_interval_lower(highest_revenue_index, &intervals);
        upper_bound = right_interval_upper(highest_revenue_index, &intervals);
        println!("search lower_bound: {:?}, upper_bound: {:?}", lower_bound, upper_bound);
    }
...
fn sanity_check(
    ...
    let frontrun_in = match pool_variant {
        PoolVariant::UniswapV2 => encode_weth(frontrun_in),
        _ => unimplemented!(),
    };
    ...
    let frontrun_out = match pool_variant {
        PoolVariant::UniswapV2 => {
            ...
            decode_intermediary(amount_out, weth_address, true, token_out)
        }
    ...
    let (frontrun_data, frontrun_value) = match pool_variant {
        ...
        PoolVariant::UniswapV3 => unimplemented!(),
    };
    ...
    let backrun_in = match pool_variant {
        PoolVariant::UniswapV2 => {
            encode_intermediary_with_dust(balance, weth_address, false, token_in)
        }
        PoolVariant::UniswapV3 => unimplemented!(),
    };
    ...
    let post_sandwich_balance =
        get_balance_of_evm(ingredients.startend_token, sandwich_contract, next_block, &mut evm)?;
...
async fn evaluate_sandwich_revenue(
    ...
    let revenue =
        post_sandwich_balance.checked_sub(*BRAINDANCE_STARTING_BALANCE).unwrap_or_default();
...
mod test {
    use crate::{
        forked_db::ForkFactory,
        mev_bots::{
            simulation::{get_from_txs, to_cache_db},
            testhelpder::{self, get_next_block_info},
            RawIngredients,
        },
    };
    use ethers::prelude::*;
    ...
    async fn create_test(
        ...
        let pool = match is_v2 {
            true => {
                testhelpder::create_v2_pool(pool_addr.parse::<Address>().unwrap(), &ws_provider)
                    .await
            }
            false => unimplemented!(),
        };
        ...
        let state =
            get_from_txs(&ws_provider, &victim_txs, BlockNumber::Number(U64::from(fork_block_num)))
                .await
                .unwrap();

        println!("state diff: ${:?}", state);
        let initial_db = to_cache_db(
            &state,
        ...
        println!("after ForkFactory::new_sandbox_factory");
        let ingredients = RawIngredients::new(
            &pool,
            victim_txs,
            weth_address,
            state,
            sandwidth_contract_info.address,
            searcher,
        )
        .await
        .unwrap();

        println!("after ingredients::new");
        match super::create_optimal_sandwich(
            &ingredients,
            ethers::utils::parse_ether("10").unwrap(),
            &get_next_block_info(fork_block_num, &ws_provider).await,
            &mut db,
            &SandwichMaker::new(
                weth_address,
                sandwidth_contract_info.address,
