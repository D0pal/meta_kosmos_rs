...
fn main() {
    ...
    let (tx_order, rx_order) = std::sync::mpsc::sync_channel::<TradeExecutionUpdate>(100);
    let (tx_wu, _rx_wu) = std::sync::mpsc::sync_channel::<WalletSnapshot>(100);
    let cefi_service = CefiService::new(Some(cex_config), None, Some(tx_order), Some(tx_wu));
...

    {
        ...
        (cefi_service_2).submit_order(
            ...
            Asset::ARB,
            Asset::USD,
            Decimal::from_f64(0.5f64).unwrap(),
        );
    }
